...
class iterative_normalization_py(torch.autograd.Function):
    ...
    def forward(ctx, *args, **kwargs):
        ...
        ctx.g = X.size(1) // nc
        x = X.transpose(0, 1).contiguous().view(nc, -1).cuda()
        d, m = x.size()
        saved = []
        if training:
            ...
            running_wmat = running_wmat.cuda()
...
class IterNorm(torch.nn.Module):
    def __init__(self, num_features, num_groups=1, num_channels=None, T=5, dim=4, eps=1e-5, momentum=0.1, affine=True,
        ...
        shape[1] = self.num_features
        if self.affine:
            self.weight = Parameter(torch.Tensor(*shape).cuda())
            self.bias = Parameter(torch.Tensor(*shape).cuda())
        else:
