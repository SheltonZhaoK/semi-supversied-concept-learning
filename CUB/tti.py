...
from CUB.dataset import find_partition_indices_by_IG, find_partition_indices_byRandom
    ...
    for _ in range(n_trials):
        ...
        if mode == 'random':
            ...
            def replace_random(attr_preds):
                ...
                # print(f'n_replace: {n_replace}, {group_replace_idx}, {replace_idx}')
                return replace_idx
        ...
        for img_id in range(len(b_class_labels)):
            attr_preds = b_attr_outputs[img_id * (args.n_attributes+args.residue): (img_id + 1) * (args.n_attributes+args.residue)]
            attr_preds_sigmoid = b_attr_outputs_sigmoid[img_id * (args.n_attributes+args.residue): (img_id + 1) * (args.n_attributes+args.residue)]
            attr_preds2 = b_attr_outputs2[img_id * (args.n_attributes+args.residue): (img_id + 1) * (args.n_attributes+args.residue)]
            if mode == 'entropy':
        ...
        attr_replace_idx = [i for i in attr_replace_idx if i < len(b_attr_binary_outputs)] #fixme
        ...
        b_attr_new = b_attr_new.reshape(-1, args.n_attributes+args.residue)
...
def parse_arguments(parser=None):
    ...
    parser.add_argument('-subset', default = '112', type=int, help="Size of the concepts to use")
    parser.add_argument('-residue', default = '0', type=int, help="Size of the residue to use")
    parser.add_argument('-disentangle', default = None, help="Vector disentanglement method")
    parser.add_argument('-reduce', default = None, help="whether to reduce concept size randomly or by importance")
...
def run(args, seed):
    ...
    class_attr_count = np.zeros((N_CLASSES, N_ATTRIBUTES, 2)) #fixme--------------------
    for d in data:
    ...
    if args.reduce == "random":
        subset_indices = find_partition_indices_byRandom(args.n_attributes, args)
    elif args.reduce == "ig":
        subset_indices = find_partition_indices_by_IG(args.n_attributes, args)
    ...
    for d in test_data:
        ...
        instance_attr_labels.extend(list(np.array(d['attribute_label'])[subset_indices]))
        uncertainty_attr_labels.extend(list(np.array(d['attribute_certainty'])[subset_indices]))
        #else:
        #instance_attr_labels.extend(list(np.array(d['attribute_label'])))
        #uncertainty_attr_labels.extend(list(np.array(d['attribute_certainty'])[indices]))
    ...
    for k, v in attr_id_to_name.items():
        ...
        if k in indices:
            class_attr_id_to_name[list(indices).index(k)] = v
    ...
    for group_id, attr_ids in attr_group_dict.items():
        ...
        for attr_id in attr_ids:
            if attr_id in indices:
            # if attr_id in mask: #fixme ---------------------
    ...
    for i in range(len(indices)):
        class_attr_id_to_name[i] = attr_id_to_name[indices[i]]
    print(class_attr_id_to_name)

    # stage 1
    _, _, _, _, b_class_labels, b_topk_class_outputs, b_class_logits, b_attr_labels, b_attr_outputs, b_attr_outputs_sigmoid, \
        b_wrong_idx, b_attr_outputs2 = eval(args, seed, None)
...
if __name__ == '__main__':
    ...
    print(output_string)
    output = open(os.path.join(args.log_dir, 'tti_results.txt'), 'w')
