...
class End2EndModel(torch.nn.Module):
    def __init__(self, model1, model2, model3, n_attributes = 112, residue = 0, use_relu=False, use_sigmoid=False, n_class_attr=2, should_detach = False, disentangle = None, mi_args = None):
        super(End2EndModel, self).__init__()
        self.first_model = model1
        self.sec_model = model2
        self.residual_model = model3
        ...
        self.mi_args = mi_args
        self.mi_estimator = mi_estimator(self.mi_args, self.disentangle, self.n_attributes, self.residue)
    ...
    def forward_stage2(self, stage1_out):
        ...
        if self.should_detach:
            if self.residue > 0:
                detached_concepts = torch.cat(stage2_inputs[:self.n_attributes], dim=1).detach()
                residue_m = torch.cat(stage2_inputs[self.n_attributes:], dim=1)
                stage2_inputs = torch.cat([detached_concepts, residue_m], dim=1)
            else:
                stage2_inputs = torch.cat(stage2_inputs, dim=1).detach()
        else:
            stage2_inputs = torch.cat(stage2_inputs, dim=1)
        
        if self.disentangle == 'IterNorm': #change T value
            ItN = IterNorm(self.n_attributes + self.residue, num_channels = self.n_attributes + self.residue, dim=2, T=7, momentum=1, affine=True)
            ItN.train()
            stage2_inputs = ItN(stage2_inputs)
            stage1_out = torch.split(stage2_inputs, 1, dim = 1)
    ...
    def forward(self, x):
        if self.first_model.training:
            ...
            if self.residual_model is not None:
                outputs_residue, aux_outputs_residue = self.residual_model(x)
                outputs.extend(outputs_residue)
                aux_outputs.extend(aux_outputs_residue)
            return self.forward_stage2(outputs), self.forward_stage2(aux_outputs)
        else:
            outputs = self.first_model(x)
            if self.residual_model is not None:
                outputs_residue = self.residual_model(x)
                outputs.extend(outputs_residue)
            #print(len(outputs))
...
class Inception3(nn.Module):
    ...
    def __init__(self, num_classes, aux_logits=True, transform_input=False, n_attributes=0, bottleneck=False, expand_dim=0, three_class=False, connect_CY=False, residue = 0, residue_indept = 0, should_detach = False):
        """
        Args:
        ...
        self.residue = residue
        self.residue_indept = residue_indept
        self.should_detach = should_detach
        ...
        if aux_logits:
            self.AuxLogits = InceptionAux(768, num_classes, n_attributes=n_attributes, bottleneck=bottleneck, residue = residue,\
                                                expand_dim=expand_dim, three_class=three_class, connect_CY=connect_CY, should_detach = should_detach)
        ...
        if self.residue_indept > 0:
            self.all_fc.append(FC(2048, residue_indept, 0))
        else:
            if self.n_attributes > 0:
                ...
                if residue > 0:
                    self.all_fc.append(FC(2048, residue, 0))
            else:
                self.all_fc.append(FC(2048, num_classes, expand_dim))
...
class InceptionAux(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, n_attributes=0, bottleneck=False, residue = 0, expand_dim=0, three_class=False, connect_CY=False, should_detach = False):
        ...
        if n_attributes > 0:
            ...
            if residue > 0:
                self.all_fc.append(FC(768, residue, expand_dim, stddev=0.001))
        else:
            self.all_fc.append(FC(768, num_classes, expand_dim, stddev=0.001))
