...
def eval(args, seed, MI_estimator):
    ...
    for data_idx, data in enumerate(loader):
        ...
        # elif args.disentangle == "IterNorm":
        #     outputs = model(inputs_var)
        #     stage2_inputs = torch.cat(outputs[1:], dim=1)
        #     ItN = IterNorm(model.n_attributes + model.residue, num_channels = model.n_attributes + model.residue, dim=2, T=7, momentum=1)
        #     ItN.train()
        #     stage2_inputs = ItN(stage2_inputs)
        #     outputs[1:] = torch.split(stage2_inputs, 1, dim = 1)
            # stage1_outs = model.first_model(inputs_var)
            # attr_outputs = stage1_outs
            ...
            if args.crossC:
                crossCorrelation = calculate_ortho_loss(concepts, concepts)
            ...
            if args.TTI > 0:
                ...
                for i in range(len(tti_concepts)): # iterate through data points
                    subset = random.sample(list(range(args.n_attributes)), int(args.n_attributes * args.TTI))
                    for index in subset:
                        if args.negTTI:
                            tti_concepts[i][index] = np.random.randn()
                        if args.posTTI:
                            tti_concepts[i][index] = attr_labels[i][index]
                # new_concepts = torch.split(tti_concepts, 1 ,dim=1)
                if args.residue > 0:
                    stage2_inputs = torch.cat([tti_concepts, residuals], dim=1)
                else:
...
def run_inference(args):
    ...
    for ratio in [i/10 for i in range(1,11)]:
        ...
        output_string = '%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n' % values
        print_string = 'Error of y: %.4f +- %.4f, Error of C: %.4f +- %.4f, Cross Correlation: %.4f +- %.4f, Mutual Information: %.4f +- %.4f\n' % values
        print(print_string)
        if args.negTTI:
            output = open(os.path.join(args.log_dir, 'negTTI.txt'), 'a')
        elif args.posTTI:
            output = open(os.path.join(args.log_dir, 'posTTI.txt'), 'a')
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-TTI', default=1, type = float, help="percentage of data to perform negative intervention")
    parser.add_argument('-dataset', default='cub', help="dataset")
    parser.add_argument('-posTTI', action = "store_true", help="perform positive itervention")
    parser.add_argument('-negTTI', action = "store_true", help="perform negtive itervention")
    ...
    if args.ray:
        ...
        # arg_list = create_args_list(args, [112])
