...
def find_partition_indices_by_IG(size, args):
    if args.dataset == "cub":  
        data = "/home/shelton/supervised-concept/CUB/output/manuscript/feature_attributions.csv"
    attributions = np.loadtxt(data, delimiter=",", dtype=float)
    indices = np.argpartition(attributions,-size)[-size:]
    return indices.tolist()

def find_partition_indices_byRandom(size, args):
    if args.dataset == "cub":
        return random.sample(range(112), size)
    if args.dataset == "ham10k":
        return random.sample(range(8), size)