...
def compute_pearsoncr(output, attribute_size):
    count = 0
    sum = 0.0
    for tensor in output:
        concept = torch.split(tensor, [attribute_size, len(tensor)-attribute_size])[0].data.cpu().numpy()
        residue = torch.split(tensor, [attribute_size, len(tensor)-attribute_size])[1].data.cpu().numpy()

        if len(residue) < len(concept):
            residue = np.pad(residue, (0, len(concept)-len(residue)), 'constant', constant_values=0)
        else:
            concept = np.pad(concept, (0, len(residue)-len(concept)), 'constant', constant_values=0)
        # print(pearsonr(concept, residue)[0])
        sum += pearsonr(concept, residue)[0]
        count += 1
    return sum / count